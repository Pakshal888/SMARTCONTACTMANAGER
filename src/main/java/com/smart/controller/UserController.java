    ...
    public String updateHandler(@ModelAttribute Contact contact,@RequestParam("profileImage") MultipartFile file,Model model,HttpSession session,Principal principle) {
        ...
        try {
            ...
            if(!file.isEmpty()) {
                ...
                File fileToDelete = new File(uploadDir, oldcontactDetails.getImage());
                if(fileToDelete.exists() && !oldcontactDetails.getImage().equals("contact.png")) {
                    fileToDelete.delete();
                }
            ...
            e.printStackTrace();
            session.setAttribute("message", new Message("Something went wrong during update !! Try again.. ","danger"));
            model.addAttribute("contact", contact); // Re-add contact to model for form re-population on error
        }
